...
void forward(Iterator begin, Iterator end) {
    ...
    static_assert(
        std::is_convertible_v<std::remove_reference_t<typename Iterator::reference>, typename Iterator::value_type>);

    static_assert(std::is_pointer_v<typename Iterator::pointer>);
    static_assert(
        std::is_convertible_v<std::remove_pointer_t<typename Iterator::pointer>, typename Iterator::value_type>);

    static_assert(
        std::is_same_v<std::remove_const_t<std::remove_reference_t<decltype(*begin)>>, typename Iterator::value_type>);
...
struct Job {
    ...
    data_feed_t range;
    test_t test;
    Job(data_feed_t r, test_t t) : range{r}, test{t} {}
};
...
void test_basic(Iterator begin, Iterator end) {
    static_assert(
        !std::is_same_v<typename std::iterator_traits<Iterator>::iterator_category(), std::input_iterator_tag>);
    static_assert(
        !std::is_same_v<typename std::iterator_traits<Iterator>::iterator_category(), std::output_iterator_tag>);
...
struct IteratorTest: public SpecificTestFixture {
    // NB! SpecificTestFixture has to provide sample data through
    // 'not_empty_container()'
};
