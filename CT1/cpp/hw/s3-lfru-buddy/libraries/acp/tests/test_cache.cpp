...
struct WithIntKey {
    ...
    WithIntKey(const int key_) : key(key_) {}

    virtual ~WithIntKey() = default;

    bool operator==(const int other_key) const { return key == other_key; }
};
...
struct StringT: WithIntKey {
    ...
    StringT(const int key_) : WithIntKey(key_), data(std::to_string(key_)) {}
...
struct Point: WithIntKey {
    ...
    static unsigned long long convert_data(const int key) { return key; }

    Point(const int key_) : WithIntKey(key_), data(key_) {}
...
struct LFRUTest: ::testing::Test {
    ...
    String& get_string(const int n) { return cache.get<String>(n); }

    Point& get_point(const int n) { return cache.get<Point>(n); }

    LFRUTest() : cache(cache_size, min_power, max_power) {}
};
