...
fun Execution.lock(id: Int) {
    ...
    for (k in 0 until N) if (k != id) {
        waitForRead(id, 3, "a[$k]")
        val l = label[k]
        checkRead(l.toString())
        my = maxOf(my, l + 1)
    }
    waitForWrite(id, 4, "a[$id]", my.toString())
    label[id] = my
    for (k in 0 until N) if (k != id) {
        while (true) {
            waitForRead(id, 7, "a[$k]")
            val other = label[k]
...
fun Execution.waitForRead(id: Int, line: Int, loc: String) {
...
fun Execution.waitForWrite(id: Int, line: Int, loc: String, value: String) {
    waitForCurThreadAction(id)
    checkLine(id, line)
    checkLocation(id, loc)
    checkValue(value)
...
private fun Execution.checkLine(id: Int, line: Int) {
    if (execution[0].line != line) {
        println("$id, Invalid line, cur=$line, expected_action=${execution[0]}")
        exitProcess(1)
    }
}
...
data class Action(
    ...
    override fun toString(): String = "$tid $line $type $loc $value"
}

enum class ExType { READ, WRITE }

private fun parseAction(s: String): Action {
    val (thread, line, action, location, value) = s.trim().split(" ")
    return Action(
        tid = thread.toThreadId(),
        line = line.toInt(),
        type = action.toExType(),
        loc = location,
        value = value
    )
}

private fun <String> String.toExType(): ExType = when (this) {
    "R" -> ExType.READ
    "W" -> ExType.WRITE
    else -> error("Invalid operation: $this")
}

private fun <String> String.toThreadId(): Int = when (this) {
    "P" -> 0
    "Q" -> 1
    else -> error("Invalid thread: $this")
}
