# Синхронизация с помощью Read-Write блокировок
...
* [`src/BankImpl.kt`](src/BankImpl.kt) содержит реализацию операций банка для однопоточного случая.
  ...
  * Альтернативная реализация на Java дана в [`java/BankImpl.java`](src/BankImpl.java).   
...
при сохранении максимального параллелизма. 

* Для реализации необходимо использовать **тонкую блокировку read-write блокировками**. 
  * Синхронизация должна осуществляться для каждого счета по отдельности. 
  * Добавьте поле `lock` типа `java.util.concurrent.locks.ReentrantReadWriteLock` (класс для примитива блокировки) в класс `BankImpl.Account`
    и используйте его для блокировки операций над соответствующим счетом.
  * В Kotlin можно использовать `lock.readLock().withLock { ... }` и `lock.writeLock().withLock { ... }`.
...
## Доработка предыдущего задания про банк

**Вы можете доработать и сдать свое решение для задания "Синхронизация с помощью тонкой блокировки".**
Отличия этого задания:

* Надо использовать `ReentrantReadWriteLock` и правильно выбирать нужный тип блокировки.
* Нужно реализовать дополнительную операцию `consolidate`.
* В этом задании более придирчивые тесты функциональности (проверяется корректность ошибок в разных ситуациях).
...
аннотацию `@Synchronized` (ключевое слово `synchronized` в Java), тем самым сделав _грубую блокировку_ или если 
реализовать задание через эксклюзивную блокировку. Однако задание требует применение тонких **read-write** блокировок.
