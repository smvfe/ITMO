...
public class BankImpl implements Bank {
     ...
     */
    @Override
    public void consolidate(@NotNull List<Integer> fromIds, int toId) {
        if (fromIds.isEmpty())
            throw new IllegalArgumentException("empty fromIds");
        if (new HashSet<>(fromIds).size() != fromIds.size())
            throw new IllegalArgumentException("duplicates in fromIds");
        if (fromIds.contains(toId))
            throw new IllegalArgumentException("toId in fromIds");
        List<Account> fromList = fromIds.stream().map(i -> accounts[i]).toList();
        Account to = accounts[toId];
        long amount = fromList.stream().mapToLong(account -> account.amount).sum();
        if (to.amount + amount > Bank.MAX_AMOUNT)
            throw new IllegalStateException("Overflow");
        for (Account from : fromList) from.amount = 0;
        to.amount += amount;
    }
