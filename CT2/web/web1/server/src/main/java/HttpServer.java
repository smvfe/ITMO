...
class HttpServer {
    ...
    private void process(Request request, Response response) throws IOException {
        ...
        String uri;
        try {
            uri = URLDecoder.decode(request.getUri().split("\\?")[0], StandardCharsets.UTF_8.name());
        } catch (IllegalArgumentException e) {
            // what if некорректные кодировки
            response.setStatusCode(400);
            response.setHeader("Content-Type", "text/plain");
            response.setBody("400 - Bad Request".getBytes());
            return;
        }
        ...
        if (file.isFile()) {
            ...
            if (etag.equals(ifNoneMatch)) {
                ...
                response.setBody("304 - Not Modified".getBytes());
                return;
            }
    ...
    private long hash(String input){
        long MOD = 1000000007, base = 31, hash = 19;
