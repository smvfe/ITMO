...
int copyout(pagetable_t pagetable, uint64 dstva, char *src, uint64 len) {
  ...
  while (len > 0) {
    ...
    if ((*pte & PTE_W) == 0 && cow(pagetable, va0)) return -1;
...
int cow(pagetable_t pagetable, uint64 va) {
  ...
  if (!(mem = kalloc())) {
    printf("COW-manager: out of memory\n");
    return -1;
  }
...
uint64 lazyalloc(struct proc * p, uint64 va){
  if (va >= p->sz || va < PGROUNDDOWN(p->trapframe->sp)) { // if the va is higher than the size or below the user stack pointer
