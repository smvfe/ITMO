...
int bit_isflip(char *array, int index) {
  index >>= 1;
  char b = array[index / 8];
  char m = (1 << (index % 8));
  return (b & m) == m;
...
void bd_free(void *p) {
  ...
  for (k = size(p); k < MAXSIZE; k++) {
    ...
    if (bit_isflip(bd_sizes[k].alloc, bi)) {  // is buddy allocated?
...
int bd_initfree_pair(int k, int bi, void *bd_ptr) {
  int buddy = (bi % 2 == 0) ? bi + 1 : bi - 1;
  int free = 0;
  if (bit_isflip(bd_sizes[k].alloc, bi)) {
    // one of the pair is free
    free = BLK_SIZE(k);
    if (bi == blk_index_next(k, bd_ptr))
...
int bd_initfree(void *bd_left, void *bd_right) {
  ...
  for (int k = 0; k < MAXSIZE; k++) {  // skip max size
    ...
    free += bd_initfree_pair(k, left, bd_left);
    if (right <= left) continue;
    free += bd_initfree_pair(k, right, bd_left);
  }
  return free;
}
