...
int dump(void) {
  ...
  printf("[DEBUG]");
  for (int i = 2; i <= 11; i++) {
    printf(" %d", *(&p->trapframe->s2 + (i - 2)));
  }
  printf("\n");
  return 0;
}

int dump2(pde_t pid, int register_num, uint64 *return_value) {
  struct proc *p;
  ...
  for (p = proc; p < &proc[NPROC]; p++) {
    ...
    if (p->pid == pid) {
      ...
      uint64 value = *(&p->trapframe->s2 + (register_num - 2));

      if (copyout(current->pagetable, (uint64)return_value, (char *)&value,
                  sizeof(value)) < 0) {
        release(&p->lock);
        return -4;
      }
